...
void COMTableWorker<Table, START_ID>::_update_s::operator()(void) const
{
    ...
    com_table.setValue(response.characteristic_id, response.data, response.index);
...
void COMTableWorker<Table, START_ID>::_upgrade_s::operator()(void) const
{
    ...
    if (!com_table.isUpdated(characteristic_id, index)) {
        fsm.push_event(success_e{});
        return;
    }
    ...
    com_table.setValue(response.characteristic_id, response.data, response.index);
    index = response.index;
    com_table.resetUpdated(characteristic_id, index);
...
void COMTableWorker<Table, START_ID>::iterate_characteristics_a::operator()(void) const
{
    ...
    if (index >= com_table.characteristicLength(characteristic_id)) {
        characteristic_id++;
        index = 0;
        errors_count = 0;
    }
